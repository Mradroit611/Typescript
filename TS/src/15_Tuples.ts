...
type Person6Info = readonly[string, number, boolean]; //using readonly so that no operation can be performed

const displayInfo = (person6:Person6Info) =>{
    const [name, age, hasDriverLicence] = person6;
    console.log(`Name: ${name}, Age: ${age}, Driver's License: ${hasDriverLicence ? "Yes": "No"}`);
}

const person65: Person6Info = ["sanjay", 22, true];
...
let person6: readonly [string, number?]; 
person6 = ['sanjay']; 
person6 = ['adroit', 22]; 
...
let values: [string, number, ...boolean[]]; // Rest elements must all be booleans
