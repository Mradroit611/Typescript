...
if (Array.isArray(val1))
    val.map(val);
if (val1 && typeof val1 === "object" && "foobar" in val1 && typeof val.foobar === "number")
    val.foobar = 4;
function login(user) {
    switch (user) {
        case "standard":
        ...
        case "superadmin":
            return true;
        default:
            const _unreachable = user; //if we comment any case then this line will start to throw an error because then it will think that the user holds the left value of User type
